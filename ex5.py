...
class Television(object):
    ...
    def set_up_channel(self):
        if self.status == 'on' and self.channel < self.number_of_channels:
                self.channel += 1

    def set_down_channel(self):
        if self.status == 'on' and self.channel > 1:
                self.channel -= 1

    def set_up_volume(self):
        if self.status == 'on' and self.volume < self.maximum_volume:
                self.volume += 1

    def set_down_volume(self):
        if self.status == 'on' and self.volume > 0:
                self.volume -= 1
